// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "./types/input";
import * as outputs from "./types/output";
import * as utilities from "./utilities";

/**
 * Creates and manages Scaleway Site-to-Site VPN Gateways.
 * For more information, see [the main documentation](https://www.scaleway.com/en/docs/site-to-site-vpn/reference-content/understanding-s2svpn/).
 *
 * ## Example Usage
 *
 * ### Basic
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as scaleway from "@pulumiverse/scaleway";
 *
 * const vpc = new scaleway.network.Vpc("vpc", {name: "my-vpc"});
 * const pn = new scaleway.network.PrivateNetwork("pn", {
 *     name: "my-private-network",
 *     vpcId: vpc.id,
 *     ipv4Subnet: {
 *         subnet: "10.0.1.0/24",
 *     },
 * });
 * const gateway = new scaleway.S2sVpnGateway("gateway", {
 *     name: "my-vpn-gateway",
 *     gatewayType: "VGW-S",
 *     privateNetworkId: pn.id,
 * });
 * ```
 *
 * ## Import
 *
 * VPN Gateways can be imported using `{region}/{id}`, e.g.
 *
 * bash
 *
 * ```sh
 * $ pulumi import scaleway:index/s2sVpnGateway:S2sVpnGateway main fr-par/11111111-1111-1111-1111-111111111111
 * ```
 */
export class S2sVpnGateway extends pulumi.CustomResource {
    /**
     * Get an existing S2sVpnGateway resource's state with the given name, ID, and optional extra
     * properties used to qualify the lookup.
     *
     * @param name The _unique_ name of the resulting resource.
     * @param id The _unique_ provider ID of the resource to lookup.
     * @param state Any extra arguments used during the lookup.
     * @param opts Optional settings to control the behavior of the CustomResource.
     */
    public static get(name: string, id: pulumi.Input<pulumi.ID>, state?: S2sVpnGatewayState, opts?: pulumi.CustomResourceOptions): S2sVpnGateway {
        return new S2sVpnGateway(name, <any>state, { ...opts, id: id });
    }

    /** @internal */
    public static readonly __pulumiType = 'scaleway:index/s2sVpnGateway:S2sVpnGateway';

    /**
     * Returns true if the given object is an instance of S2sVpnGateway.  This is designed to work even
     * when multiple copies of the Pulumi SDK have been loaded into the same process.
     */
    public static isInstance(obj: any): obj is S2sVpnGateway {
        if (obj === undefined || obj === null) {
            return false;
        }
        return obj['__pulumiType'] === S2sVpnGateway.__pulumiType;
    }

    /**
     * The AS Number of the VPN gateway (typically 12876 for Scaleway).
     */
    declare public /*out*/ readonly asn: pulumi.Output<number>;
    /**
     * The date and time of the creation of the VPN gateway (RFC 3339 format).
     */
    declare public /*out*/ readonly createdAt: pulumi.Output<string>;
    /**
     * The VPN gateway type (commercial offer type).
     */
    declare public readonly gatewayType: pulumi.Output<string>;
    /**
     * The ID of the IPAM private IPv4 address to attach to the VPN gateway.
     */
    declare public readonly ipamPrivateIpv4Id: pulumi.Output<string>;
    /**
     * The ID of the IPAM private IPv6 address to attach to the VPN gateway.
     */
    declare public readonly ipamPrivateIpv6Id: pulumi.Output<string>;
    /**
     * The name of the VPN gateway. If not provided, it will be randomly generated.
     */
    declare public readonly name: pulumi.Output<string>;
    /**
     * The Organization ID the VPN gateway is associated with.
     */
    declare public /*out*/ readonly organizationId: pulumi.Output<string>;
    /**
     * The ID of the Private Network to attach to the VPN gateway.
     */
    declare public readonly privateNetworkId: pulumi.Output<string>;
    /**
     * `projectId`) The ID of the project the VPN gateway is associated with.
     */
    declare public readonly projectId: pulumi.Output<string>;
    /**
     * The public endpoint configuration of the VPN gateway. See Public Config below.
     */
    declare public readonly publicConfigs: pulumi.Output<outputs.S2sVpnGatewayPublicConfig[]>;
    /**
     * `region`) The region in which the VPN gateway should be created.
     */
    declare public readonly region: pulumi.Output<string | undefined>;
    /**
     * The status of the VPN gateway.
     */
    declare public /*out*/ readonly status: pulumi.Output<string>;
    /**
     * The list of tags to apply to the VPN gateway.
     */
    declare public readonly tags: pulumi.Output<string[]>;
    /**
     * The date and time of the last update of the VPN gateway (RFC 3339 format).
     */
    declare public /*out*/ readonly updatedAt: pulumi.Output<string>;
    /**
     * `zone`) The zone in which the VPN gateway should be created.
     */
    declare public readonly zone: pulumi.Output<string>;

    /**
     * Create a S2sVpnGateway resource with the given unique name, arguments, and options.
     *
     * @param name The _unique_ name of the resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param opts A bag of options that control this resource's behavior.
     */
    constructor(name: string, args: S2sVpnGatewayArgs, opts?: pulumi.CustomResourceOptions)
    constructor(name: string, argsOrState?: S2sVpnGatewayArgs | S2sVpnGatewayState, opts?: pulumi.CustomResourceOptions) {
        let resourceInputs: pulumi.Inputs = {};
        opts = opts || {};
        if (opts.id) {
            const state = argsOrState as S2sVpnGatewayState | undefined;
            resourceInputs["asn"] = state?.asn;
            resourceInputs["createdAt"] = state?.createdAt;
            resourceInputs["gatewayType"] = state?.gatewayType;
            resourceInputs["ipamPrivateIpv4Id"] = state?.ipamPrivateIpv4Id;
            resourceInputs["ipamPrivateIpv6Id"] = state?.ipamPrivateIpv6Id;
            resourceInputs["name"] = state?.name;
            resourceInputs["organizationId"] = state?.organizationId;
            resourceInputs["privateNetworkId"] = state?.privateNetworkId;
            resourceInputs["projectId"] = state?.projectId;
            resourceInputs["publicConfigs"] = state?.publicConfigs;
            resourceInputs["region"] = state?.region;
            resourceInputs["status"] = state?.status;
            resourceInputs["tags"] = state?.tags;
            resourceInputs["updatedAt"] = state?.updatedAt;
            resourceInputs["zone"] = state?.zone;
        } else {
            const args = argsOrState as S2sVpnGatewayArgs | undefined;
            if (args?.gatewayType === undefined && !opts.urn) {
                throw new Error("Missing required property 'gatewayType'");
            }
            if (args?.privateNetworkId === undefined && !opts.urn) {
                throw new Error("Missing required property 'privateNetworkId'");
            }
            resourceInputs["gatewayType"] = args?.gatewayType;
            resourceInputs["ipamPrivateIpv4Id"] = args?.ipamPrivateIpv4Id;
            resourceInputs["ipamPrivateIpv6Id"] = args?.ipamPrivateIpv6Id;
            resourceInputs["name"] = args?.name;
            resourceInputs["privateNetworkId"] = args?.privateNetworkId;
            resourceInputs["projectId"] = args?.projectId;
            resourceInputs["publicConfigs"] = args?.publicConfigs;
            resourceInputs["region"] = args?.region;
            resourceInputs["tags"] = args?.tags;
            resourceInputs["zone"] = args?.zone;
            resourceInputs["asn"] = undefined /*out*/;
            resourceInputs["createdAt"] = undefined /*out*/;
            resourceInputs["organizationId"] = undefined /*out*/;
            resourceInputs["status"] = undefined /*out*/;
            resourceInputs["updatedAt"] = undefined /*out*/;
        }
        opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts);
        super(S2sVpnGateway.__pulumiType, name, resourceInputs, opts);
    }
}

/**
 * Input properties used for looking up and filtering S2sVpnGateway resources.
 */
export interface S2sVpnGatewayState {
    /**
     * The AS Number of the VPN gateway (typically 12876 for Scaleway).
     */
    asn?: pulumi.Input<number>;
    /**
     * The date and time of the creation of the VPN gateway (RFC 3339 format).
     */
    createdAt?: pulumi.Input<string>;
    /**
     * The VPN gateway type (commercial offer type).
     */
    gatewayType?: pulumi.Input<string>;
    /**
     * The ID of the IPAM private IPv4 address to attach to the VPN gateway.
     */
    ipamPrivateIpv4Id?: pulumi.Input<string>;
    /**
     * The ID of the IPAM private IPv6 address to attach to the VPN gateway.
     */
    ipamPrivateIpv6Id?: pulumi.Input<string>;
    /**
     * The name of the VPN gateway. If not provided, it will be randomly generated.
     */
    name?: pulumi.Input<string>;
    /**
     * The Organization ID the VPN gateway is associated with.
     */
    organizationId?: pulumi.Input<string>;
    /**
     * The ID of the Private Network to attach to the VPN gateway.
     */
    privateNetworkId?: pulumi.Input<string>;
    /**
     * `projectId`) The ID of the project the VPN gateway is associated with.
     */
    projectId?: pulumi.Input<string>;
    /**
     * The public endpoint configuration of the VPN gateway. See Public Config below.
     */
    publicConfigs?: pulumi.Input<pulumi.Input<inputs.S2sVpnGatewayPublicConfig>[]>;
    /**
     * `region`) The region in which the VPN gateway should be created.
     */
    region?: pulumi.Input<string>;
    /**
     * The status of the VPN gateway.
     */
    status?: pulumi.Input<string>;
    /**
     * The list of tags to apply to the VPN gateway.
     */
    tags?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * The date and time of the last update of the VPN gateway (RFC 3339 format).
     */
    updatedAt?: pulumi.Input<string>;
    /**
     * `zone`) The zone in which the VPN gateway should be created.
     */
    zone?: pulumi.Input<string>;
}

/**
 * The set of arguments for constructing a S2sVpnGateway resource.
 */
export interface S2sVpnGatewayArgs {
    /**
     * The VPN gateway type (commercial offer type).
     */
    gatewayType: pulumi.Input<string>;
    /**
     * The ID of the IPAM private IPv4 address to attach to the VPN gateway.
     */
    ipamPrivateIpv4Id?: pulumi.Input<string>;
    /**
     * The ID of the IPAM private IPv6 address to attach to the VPN gateway.
     */
    ipamPrivateIpv6Id?: pulumi.Input<string>;
    /**
     * The name of the VPN gateway. If not provided, it will be randomly generated.
     */
    name?: pulumi.Input<string>;
    /**
     * The ID of the Private Network to attach to the VPN gateway.
     */
    privateNetworkId: pulumi.Input<string>;
    /**
     * `projectId`) The ID of the project the VPN gateway is associated with.
     */
    projectId?: pulumi.Input<string>;
    /**
     * The public endpoint configuration of the VPN gateway. See Public Config below.
     */
    publicConfigs?: pulumi.Input<pulumi.Input<inputs.S2sVpnGatewayPublicConfig>[]>;
    /**
     * `region`) The region in which the VPN gateway should be created.
     */
    region?: pulumi.Input<string>;
    /**
     * The list of tags to apply to the VPN gateway.
     */
    tags?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * `zone`) The zone in which the VPN gateway should be created.
     */
    zone?: pulumi.Input<string>;
}
